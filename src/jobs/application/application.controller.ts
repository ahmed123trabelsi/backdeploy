...
import { CvData } from './cvData.schemas';
...
//       }
  ...
  async generateCv(@Body() cvData: CvData, @Res() res: Response): Promise<void> {
    ...
    const browser = await puppeteer.launch({
      executablePath: 'C:\Program File\Google\Chrome\Application\chrome.exe'
    });
      ...
      <head>
          ...
          <style>
              ...
              .contact-info {
                  ...
                  border-right: 1px solid #ccc; /* Ajout d'une bordure pour séparer */
                  padding-right: 20px; /* Espacement pour les coordonnées */
                  margin-right: 20px; /* Espacement entre les sections */
              }
      
              .contact-info h3 {
                  margin-top: 0; /* Suppression de la marge pour le titre */
              }
      
              .content {
                  flex: 1; /* L'expérience et les compétences occupent le reste */
              }
      ...
      <body>
          ...
          <hr>
                  ...
                  <div class="skills">
                      <h3>Compétences</h3>
                      <ul>
                          ${cvData.competences.split(',').map(competence => `<li>${competence.trim()}</li>`).join('')}
                      </ul>
                  </div>
              </div>
          </div>
